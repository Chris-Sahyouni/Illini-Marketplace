...
export default function Page() {
    ...
    useEffect(() => {
        ...
        wrapper();
    }, [session?.user.id, toggleReload]);

    const handleDelete = async (e: React.MouseEvent<HTMLButtonElement>) => {
        const fullItemId = e.currentTarget.getAttribute('id')
        const targetId = fullItemId?.split('-')[0]
        const type = items.find((item) => item.id === targetId)?.type

        console.log("deleting: " + targetId + ' of type: ' + type)

        await deleteItem(targetId, type).then((res) => {
            console.log("RES: " + res);
            if (res) setToggleReload(!toggleReload);
        })
    }
